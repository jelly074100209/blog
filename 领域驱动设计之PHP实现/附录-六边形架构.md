...
你当前的 API 由基于 Symfony2 组件的 PHP 迷你框架 Silex 构建的，让我们看看清单 7：
...
这仍然有很工作要做。正如你可能听说过的，一个真正的工匠会使用 TDD （测试驱动开发）。我们已经开始了我们的故事，因此我们必须在之后的测试保持正确。

## 评级 idea 测试用例

Michael Feathers 引入了遗留代码（legacy code）的定义，即没有测试的代码。你并不希望你的代码刚诞生就成为遗留代码，对吗？
...
这里，我们使用了相同的策略但是用了一个 EmptyIdeaRepository。它同样实现了相同的接口，但无论 finder 方法接收什么标识，这个实现总是返回一个 null。
...
如果你想对应用进行 100% 的测试，则需要测试你的基础设施。在做这个之前，你需要知道这些单元测试会比业务更耦合你的实现。这意味着用实现细节的改变的破坏可能性变得更高。因此这点需要你权衡考虑。

那么，如果你想继续，我们需要做一些修改。我们需要解耦更多。让我们看看清单 13：
...
如果我们想对 RedisIdeaRepository 进行 100% 的单元测试，则需要在不指定　TypeHinting 的情况下把 Predis\Client 作为一个参数传递给仓储，以便我们可以传递一个 mock 来使得必要代码流程覆盖所有用例。
